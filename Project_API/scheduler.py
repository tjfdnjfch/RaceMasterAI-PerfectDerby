...
def Todo():
    ...
    for table in tablelist:
        if table == '출전_등록말_정보':
            ...
            end_date0= datetime.datetime.today().strftime("%Y%m%d")
            MPlist = [str(p) for p in MatchingPeriod if (p > int(maxMP1)) and (p < int(end_date0))]
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                for date in MPlist:
                    time.sleep(0.3)
            ...
            maxMP =  pd.read_sql_query(f"SELECT MAX(MatchingPeriod)  FROM {table};", conn).to_string(index=False, header=False)
            MPlist = [str(p) for p in MatchingPeriod if (p > int(maxMP))]
            f = open("abc.txt", 'w', encoding="utf8")
            f.write(f"{table} : {MPlist}")
            f.close()
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                for date in MPlist:
                    time.sleep(0.3)
    ...
    df.drop_duplicates(subset=['MatchingPeriod', 'rcNo', 'hrNo'])
